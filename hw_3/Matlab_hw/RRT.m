...
for iter = 1:3000
    ...
    [x_near, dis] = findNearest(T, count, x_rand(1), x_rand(2))
    %x_near = node_dis(1)
    %dis = node_dis(2)
    ...
    %Step 3: 扩展得到x_new节点
    %提示：注意使用扩展步长Delta
    x_new = try_rand_node([x_near.x, x_near.y], x_rand, Delta)
    ...
    if ~collisionChecking([x_near.x, x_near.y],x_new,Imp) 
        continue;
    end
    count=count+1;
    ...
    T.v(count).indPrev = count-1;     % index of parent?
    
    %Step 5:检查是否到达目标点附近 
    %提示：注意使用目标点阈值Thr，若当前节点和终点的欧式距离小于Thr，则跳出当前for循环
    if computeNodeDist(x_new(1),x_new(2), x_G, y_G ) <= Thr
        disp("Met node around goal")
        plot(x_near(1), x_near(2),   x_new(1), x_new(2),  'b', 'Linewidth', 3);
        hold on
        break;
    end
...
function [nearest_nodexxx, nearest_node_dis] =findNearest(T, t_size,x, y)
    ret_n_node = T.v(1)
    min_distance = inf
    for i = 1:t_size
    ...
    nearest_nodexxx = ret_n_node

    nearest_node_dis = min_distance
