...
def build_up_graph(grid, save_path):
    ...
    for p_idx in range(coord.shape[0]):
        ...
        for vel in vel_list:
            ...
            # 
            heuristic = m_dist[:, 0] + m_dist[:, 1]
            #heuristic = np.linalg.norm(m_dist, axis=1)
            
            # diagonal distance as h .
            #h_all = []
            #for pnt in FINISH_LINE:
            #    diff_x = abs(pnt[0] - state.px)
            #    diff_y = abs(pnt[1] - state.py)
            #    tmp = min([diff_y, diff_x])
            #    dis1 = math.sqrt(2*tmp*tmp)
            #    dis2 = abs((diff_x - tmp)) + abs((diff_y - tmp))
            #    h_all.append(dis2 + dis1)
#
            #heuristic = min(h_all)
    ...
    for i in range(500): # YOU MAY CHANGE THIS VALUE
        ...
        for key in greedy_plan:
            ...
            if state.is_goal:
                ...
                bellman_error = bellman_error +  np.linalg.norm(state.g_value - current_value)
...
if __name__ == '__main__':
    ...
    if action == "build_g":
        ...
        #track_map = race_track
        path = './solution/graph_rtdp_dia.dat'
